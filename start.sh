...
if [ "$SERVER_READY" = true ]; then
    ...
    MCP_PORT_PID=$(lsof -ti:8104 || true)
    if [ -n "$MCP_PORT_PID" ]; then
        echo -e "${YELLOW}⚠️  MCP middleware already running on port 8104 (PID: $MCP_PORT_PID). Skipping...${NC}"
    else
        ...
        if lsof -ti:8104 > /dev/null 2>&1; then
            echo -e "${GREEN}✓ MCP middleware is ready on port 8104${NC}"
        else
    ...
    if [ -f ".middleware.pid" ]; then
        ...
        echo -e "MCP Middleware: ${YELLOW}PID $MCP_DISPLAY_PID${NC} | ${BLUE}http://localhost:8104${NC} | ${YELLOW}Logs: /tmp/mcp_middleware.log${NC}"
    fi
    echo ""
